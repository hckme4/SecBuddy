...
printf "GENTOO UPDATE\nRun This As Root!!!\n-----------------------------------------\n";
printf "\nWARNING: Updating a Gentoo System involves recompiling every package on it.\n";
...
do
	...
	then
		...
		# Begin with updating world set.
		emerge --deep @world --autounmask-write
		etc-update
		emerge --deep @world

		# Update system
		emerge --deep @system --autounmask-write
		etc-update
		emerge --deep @system
